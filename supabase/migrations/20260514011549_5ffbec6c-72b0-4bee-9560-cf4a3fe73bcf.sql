...
BEGIN
  ...
  -- Master TA bootstrap (only this email)
  ...
  CASE upper(v_account_type)
    ...
    WHEN 'TA'    THEN v_tenant_type := 'TA';    v_role := 'ta_staff';
    ELSE RETURN NEW;
  END CASE;

  -- TA staff: attach to existing root TA tenant instead of creating new
  IF v_tenant_type = 'TA' THEN
    SELECT id INTO ta_tenant_id FROM public.tenants WHERE type = 'TA' LIMIT 1;
    IF ta_tenant_id IS NOT NULL THEN
      INSERT INTO public.user_roles (user_id, tenant_id, role)
      VALUES (NEW.id, ta_tenant_id, 'ta_staff')
      ON CONFLICT DO NOTHING;
      UPDATE public.profiles SET primary_tenant_id = ta_tenant_id WHERE id = NEW.id;
    END IF;
    RETURN NEW;
